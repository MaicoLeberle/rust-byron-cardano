...
mod tests {
    ...
    fn test_verify_signature() {
        ...
        let witness_cbor =
            "8200D8185885825840888CDF85991D85F2023423BA4C80D41570EBF1FC878C9F5731DF1D20C64AECF3E8AA\
            2BBAFC9BEBA8EF33ACB4D7E199B445229085718FBA83B7F86AB6A3BCF782584063E34CF5FA6D8C028863043\
            7FA5E151D93907E826E66BA273145E3EE712930B6F446FF81CB91D7F0CB4CECCD0466BA9AB14448D7EAB9FC\
            480A122324BD80170E";
        match hex::decode(witness_cbor) {
            Ok(bytes) =>
                match pallas::codec::minicbor::decode::
                        <pallas::ledger::primitives::byron::Twit>(&bytes[..]) {
                    Ok(pallas::ledger::primitives::byron::Twit::PkWitness(wit_cborwrap)) => {
                        ...
                        match hex::decode(tx_hash_hex) {
                            Ok(tx_hash) => {
                                let prot_magic: ProtocolMagic = ProtocolMagic::new(
                                    764824073 as u32
                                );
                                let signing_tag: u64 = SigningTag::Tx as u64;
                                let mut se = Serializer::new_vec();
                                se.write_unsigned_integer(signing_tag)
                                    .expect("write the sign tag")
                                    .serialize(&prot_magic)
                                    .expect("serialize protocol magic")
                                    .serialize(&tx_hash.as_slice());
                                let mut data_to_verify: Vec<u8> = se.finalize();
                                assert!(
                                    ed25519::verify(&data_to_verify, &pub_key, &signature),
                                    "Unable to verify signature"
                                );
                            },
                            Err(_) => panic!("Transaction hash couldn't be decoded."),
                        }
                    },
